...
class KeywordProcessor(object):
    ...
    def __contains__(self, word):
        ...
        print("return", self._keyword in current_dict and len_covered == len(word))
    ...
    def __delitem__(self, keyword):
        ...
        if keyword: #1 GACC tmam byd5ol lma ykon fadya aw la
            if not self.case_sensitive:#2CACC tmam by5ol lma case sensetive b false aw true
                keyword = keyword.lower()
            current_dict = self.keyword_trie_dict
            character_trie_list = []
            
            for letter in keyword:#3 GACC  tmam byd5ol lma ykon fadya aw la
                if letter in current_dict:#4CACC byd5ol lw awl 7arf mwgod fe el dic
            ...
            if current_dict and self._keyword in current_dict:#5RICC  y2ma TT aw TF
                ...
                for key_to_remove, dict_pointer in character_trie_list:#6 GICC
                    
                    print(key_to_remove)
                    print(dict_pointer)
                    if len(dict_pointer.keys()) == 1:#7 GACC TMAM
    ...
    def extract_keywords(self, sentence, span_info=False):
        ...
        if not self.case_sensitive: #done
        ...
        while idx < sentence_len: #done
            
            char = sentence[idx]
            # when we reach a character that might denote word end
            if char not in self.non_word_boundaries: #tmam
                # if end is present in current_dict
                if self._keyword in current_dict or char in current_dict:#tmam
                    # update longest sequence found
                    sequence_found = None
                    longest_sequence_found = None
                    is_longer_seq_found = False
                    if self._keyword in current_dict:#m4 la2ya ay case bt722ha !!
                    ...
                    if char in current_dict:
                        ...
                        idy = idx + 1
                        while idy < sentence_len:#tmam
                            inner_char = sentence[idy]
                            if inner_char not in self.non_word_boundaries and self._keyword in current_dict_continued:#tmam
                                # update longest sequence found
                                longest_sequence_found = current_dict_continued[self._keyword]
                                sequence_end_pos = idy
                                is_longer_seq_found = True
                            if inner_char in current_dict_continued: #tmam
                            ...
                            if self._keyword in current_dict_continued:#tmam
                                # update longest sequence found
                                longest_sequence_found = current_dict_continued[self._keyword]
                                sequence_end_pos = idy
                                is_longer_seq_found = True
                        if is_longer_seq_found:#tmam
                            idx = sequence_end_pos
                    current_dict = self.keyword_trie_dict
                    if longest_sequence_found:#tmam
                    ...
                    current_dict = self.keyword_trie_dict
                    reset_current_dict = True
            elif char in current_dict:#tmam
        ...
        print("span_info = ",span_info)
        if span_info:
            print("output span= ", [value[0] for value in keywords_extracted])
            return keywords_extracted
        
        print("output a5r el func= ", [value[0] for value in keywords_extracted])
...
keyword_processor = KeywordProcessor()
keyword_processor.add_keyword('Big Apple')
#keyword_processor.__contains__('Big Apple')  #d5l bs return true
#keyword_processor.__contains__('Big Area') #da5l bs el return false
#keyword_processor.__contains__('Area Big') #ةشda5l w el return false
